package com.bimotv.app.ui.favorites
...
class FavoritesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, state: Bundle?) { binding.title.text = getString(R.string.favorites); adapter = ChannelAdapter(::open) { viewModel.toggleFavorite(it.id) }; binding.recycler.layoutManager = GridLayoutManager(requireContext(), if (resources.configuration.screenWidthDp >= 600) 3 else 2); binding.recycler.adapter = adapter; viewLifecycleOwner.lifecycleScope.launch { repeatOnLifecycle(Lifecycle.State.STARTED) { launch { viewModel.favoriteIds.collect { adapter.setFavoriteIds(it) } }; launch { viewModel.channels.collect { adapter.submitList(it.filter { c -> c.id in viewModel.favoriteIds.value }) } } } } }
